  ...
  for(int k = 0; k < ecc_len; k++){
        ...
        if(delta != 0){
            ...
            for(int j = 0; j < size; j++){
                int target = j + shift;
                if(target < size){
                    lambda[target] = gf_add(lambda[target],gf_multiply(gf_divide(delta, b), bestL[j]));
                }
            }
    ...
    int last = size - 1;
    while(last > 0 && lambda[last] == 0) last--;
...
int* chien_search(uint8_t* lambda, int lambda_len, int codeword_len, int* out_count){
    int* positions = malloc((lambda_len - 1) * sizeof(int)); // at most degree(λ) errors
    *out_count = 0;
    for(int i = 0; i < 255; i++){
...
uint8_t* forney(uint8_t* lambda, int lambda_len,uint8_t* syndromes, int ecc_len,uint8_t* codeword, int codeword_len,int* error_positions, int error_count){
    ...
    int out_len_omega;    
    int lambda_len_der;
    uint8_t* omega = poly_multiply(syndromes_poly, ecc_len,lambda,lambda_len,&out_len_omega); //heap-allocated
    ...
    for(int i = 0; i < ecc_len; i++){
        omaga_scaled[i] = omega[out_len_omega - ecc_len + i]; // take LAST ecc_len terms
    }
    } else {
        omaga_scaled  = omega;
    ...
    for(int i =0; i < error_count; i++){
        ...
        codeword[error_positions[i]] = gf_add(codeword[error_positions[i]], error_mang);
    }
    if(omaga_scaled != omega) free(omaga_scaled);
        free(omega);
    free(lambda_der);
    return codeword;
}
